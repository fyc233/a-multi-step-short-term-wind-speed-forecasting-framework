...
# 多变量预测train_X = np.reshape(train_X, (train_X.shape[0], embedding_dim, 1))中第一位不变，第二位变成每几个预测未来一个值，第三位为几个因素
...
model = Sequential()
# model.add(Bidirectional(LSTM(50, activation='relu'), input_shape=(embedding_dim, 1)))
model.add(Bidirectional(LSTM(50, activation='relu'), input_shape=(embedding_dim, 1)))
model.add(Dense(delay)) ##几步预测就填几（三步预测：model.add(Dense(3))）
model.compile(optimizer='adam', loss='mse')

# train the model
# model.fit(train_X, train_y, epochs=50, batch_size=32, verbose=2)
model.fit(train_X, train_y, epochs=30, batch_size=2, verbose=1)
# make predictions
